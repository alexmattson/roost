/* Sandpiper Analytics — data normalisation + metric derivation.
...
// Order matters: the first matching rule wins, so the least ambiguous buckets go first.
const CATEGORY_RULES = [
  rule('Lighting', ['lamp', 'lantern', 'sconce', 'candelabra', 'candelabrum', 'candlestick', 'candle', 'chandelier']),
  rule('Furniture', ['chair', 'table(?! tennis)', 'stool', 'bench', 'dresser', 'hutch', 'cabinet', 'chest', 'shelf', 'shelves',
    'bed', 'ottoman', 'trunk', 'desk', 'rocker', 'drawer', 'secretary', 'sideboard', 'armoire']),
  rule('Rugs & Textiles', ['rug', 'quilt', 'pillow', 'pillowcase', 'blanket', 'poncho', 'textile', 'cowhide',
    'linen', 'tapestry', 'throw']),
  rule('Art & Wall Decor', ['painting', 'portrait', 'artwork', 'art', 'print', 'frame', 'landscape', 'embroidery',
    'mirror', 'still life', 'canvas']),
  rule('Baskets & Wicker', ['basket', 'wicker', 'rattan', 'hamper']),
  rule('Pottery & Ceramics', ['pottery', 'ceramic', 'stoneware', 'porcelain', 'crock', 'vase', 'vessel', 'jug',
    'pitcher', 'mug', 'goblet', 'bowl', 'planter', 'terracotta', 'jar', 'teapot', 'cachepot', 'creamer', 'dish',
    'amphora', 'demijohn', 'pot']),
  rule('Kitchen & Barware', ['rolling pin', 'mold', 'mould', 'mortar', 'pestle', 'butter', 'tray', 'coaster',
    'jigger', 'opener', 'napkin', 'washboard', 'kettle', 'bellows', 'spoon', 'grinder', 'chiller', 'scale',
    'trivet', 'ashtray']),
  rule('Sporting & Games', ['tennis', 'baseball', 'badminton', 'croquet', 'snow ?shoe', 'pinball', 'game',
    'racket', 'bat', 'paddle', 'decoy', 'sled']),
  rule('Toys & Children', ['doll', 'toy', 'kid', 'child', 'children', 'ride-on', 'clown', 'piggybank',
    'rocking horse', 'block']),
  rule('Metalware', ['brass', 'copper', 'bronze', 'pewter', 'silver', 'cast iron', 'metal', 'tin', 'onyx', 'marble']),
  rule('Storage & Containers', ['box', 'bin', 'crate', 'holder', 'rack', 'hook', 'caddy'])
];
...
const dowIndex = (t) => (new Date(t).getDay() + 6) % 7;
...
export function analyze(items, range, venue = null) {
  ...
  const dayOfWeek = DOW_LABELS.map((label, idx) => {
    const rows = sales.filter((i) => dowIndex(i.sold) === idx);
    return {
      label,
      units: rows.length,
      gross: sum(rows, (i) => i.soldPrice),
      net: sum(rows, (i) => i.net),
      profit: sum(rows, (i) => i.profit)
    };
  });

  /**
   * Share of sales recorded within two minutes of another. Sandpiper stamps `sold`
   * when the sale is entered, so a high ratio means batch data entry and the
   * weekday split reflects bookkeeping rhythm, not when customers actually bought.
   */
  const batchRatio = (() => {
    const times = sales.map((i) => i.sold).sort((a, b) => a - b);
    if (times.length < 2) return 0;
    let clustered = 0;
    for (let i = 0; i < times.length; i += 1) {
      const nearPrev = i > 0 && times[i] - times[i - 1] <= 120000;
      const nearNext = i < times.length - 1 && times[i + 1] - times[i] <= 120000;
      if (nearPrev || nearNext) clustered += 1;
    }
    return clustered / times.length;
  })();

  // Same-length window immediately before this one, for period-over-period deltas.
  const span = Math.max(1, end - start);
  const prevSales = items.filter((i) => i.sold != null && i.sold >= start - span && i.sold < start).filter(matchesVenue);
  const prevAcquired = items.filter((i) => i.acquired != null && i.acquired >= start - span && i.acquired < start);
  const previous = {
    ...summarizeSales(prevSales),
    spend: sum(prevAcquired, (i) => i.cost),
    acquiredUnits: prevAcquired.length,
    hasData: prevSales.length > 0 || prevAcquired.length > 0
  };

  const lastSaleAt = items.reduce((max, i) => (i.sold != null && i.sold > max ? i.sold : max), 0) || null;
  const weeks = span / DAY / 7;

  return {
    dayOfWeek,
    bestWeekday: dayOfWeek.reduce((a, b) => (!a || b.net > a.net ? b : a), null),
    batchRatio,
    previous,
    lastSaleAt,
    perWeek: weeks > 0 ? sales.length / weeks : null,
    biggestSale: sales.reduce((a, b) => (!a || b.soldPrice > a.soldPrice ? b : a), null),
    recentSales: [...sales].sort((a, b) => b.sold - a.sold).slice(0, 8),
    recentAcquisitions: [...acquisitions].sort((a, b) => b.acquired - a.acquired).slice(0, 8),
