...
/**
 * Signs in. Sandpiper is required; Quail is optional but strongly wanted, since
 * without it there is no register truth, no rent, and no Sync. A Quail failure
 * is returned rather than thrown, so a good Sandpiper login is never lost to it.
 */
export async function webLogin({ sandpiperToken, quail }) {
  /* Sandpiper's data API sends CORS and takes a Bearer token, but its login
   * route sends none — so a browser on another origin cannot exchange a
   * password for a token, only use one it already holds. sessionFromToken
   * decodes it and rejects an expired one before anything is stored. */
  const token = String(sandpiperToken || '').trim();
  if (!token) throw new Error('Paste your Sandpiper session token to sign in.');
  let s;
  try {
  ...
  let quailEmail = null;
  let quailError = null;
  if (quail && quail.email && quail.password) {
    try {
      const q = await core.loginQuail({ request, email: quail.email, password: quail.password });
      quailEmail = q.email;
      quailAuth = core.quailAuthHeader(q.email, q.sessionId);
    } catch (e) {
  ...
  session = {
    sandpiperToken: token,
    accounts: s.accounts,
    username: s.username,
    accountId: s.accounts[0] || null,
    quailAuth,
    quailEmail
  };

  // Prove the token works before committing to it: one cheap authenticated call,
  // so a token that the API rejects fails here with a clear message.
  try {
    await core.fetchVenues({ request, accountId: session.accountId, token });
  } catch (e) {
    session = null;
    if (e.status === 401 || e.status === 403) {
      throw new Error('Sandpiper rejected that token. Copy a fresh one — see “Where do I find this?”.');
    }
    throw e;
  }

  persist();
  return { session, quailError, quailConnected: !!quailAuth };
}
