...
export function reconcile(items, quailSales, range) {
  ...
  for (const sale of quailInRange) {
    ...
    if (Math.abs(pick.soldPrice - sale.price) > CENT_TOLERANCE) {
      add({
        ...
        soldAt: sale.soldAt,
        inv: sale.inv,
        detail: `#${sale.inv} "${pick.desc}": Sandpiper ${pick.soldPrice}, Quail ${sale.price}`,
    ...
    if (Math.abs(pick.commission - sale.consignment) > CENT_TOLERANCE) {
      add({
        ...
        soldAt: sale.soldAt,
        inv: sale.inv,
        detail: `#${sale.inv} "${pick.desc}": commission ${pick.commission} vs Quail ${sale.consignment}`,
  ...
  for (const sale of untagged) {
    ...
    add({
      ...
      soldAt: sale.soldAt,
      inv: candidate.inv,
      detail: `Untagged Quail sale "${sale.desc}" (${sale.price}) most likely is Sandpiper #${candidate.inv} "${candidate.desc}"`,
      quail: sale,
      item: candidate,
  ...
  for (const sale of untagged) {
    ...
    add({
      ...
      severity: 'medium',
      soldAt: sale.soldAt,
      detail: `Sold "${sale.desc}" for ${sale.price} with no inventory number`,
      quail: sale,
