...
/** Sandpiper login: username + password in, a JWT out — the endpoint is under
 *  /api, which sends CORS, so this works from any origin. */
export async function loginSandpiper({ request, username, password }) {
  const res = await request(`${SANDPIPER}/api/login/do-login`, null, {
    method: 'POST', body: { username, password }, auth: false
  });
  const token = res && (res.jwtToken || res.token || (res.user && res.user.jwtToken));
  if (!token) throw new Error('Sandpiper accepted the sign-in but returned no session token.');
  return token;
}
